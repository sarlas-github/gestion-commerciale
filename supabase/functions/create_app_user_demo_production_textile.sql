-- Dernière version déployée : 20260604160000_OBJETS_create_app_user_demo_production_textile.sql
-- Usage : SELECT * FROM public.create_app_user_demo_production_textile('demo@prospect.ma');
--         SELECT * FROM public.create_app_user_demo_production_textile('demo@prospect.ma', p_label_quantity := 'Mètres', p_show_pieces_count := false);
-- Flux production : achats MP (tissus, fils, accessoires) → ordres de confection (consommation MP + fabrication PF) → ventes PF (vêtements)
...
BEGIN
    ...
    -- Caftan Femme brodé (index 13, seuil=5) → rupture — collection vendue
    v_target := 0;
