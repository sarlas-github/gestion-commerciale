-- Fonction atomique create_receipt
-- Toutes les opérations (séquence + document + items) dans une seule transaction.
-- Si l'INSERT documents ou document_items échoue → rollback complet,
-- y compris le UPDATE document_sequences (pas de trou dans la numérotation).
--
-- p_items : JSONB array d'objets { product_id, product_name, quantity (total pièces), unit_price }
...
BEGIN
  ...
  --    Si l'étape 3 plante, ce UPDATE est aussi rollbacké → aucun trou possible.
  ...
  -- 2. Snapshot entreprise
  SELECT * INTO v_company
  FROM companies
  WHERE user_id = v_user_id
  LIMIT 1;

  -- 3. Insertion document
