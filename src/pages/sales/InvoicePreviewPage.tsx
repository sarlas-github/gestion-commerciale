...
export const InvoicePreviewPage = () => {
  ...
  return (
    <div className="flex flex-col h-full -m-4 lg:-m-6">
      <PageHeader
        title={existingInvoice ? `Facture ${existingInvoice.number}` : 'Aperçu facture — brouillon'}
        leftAction={
        ...
        actions={
          !isLoading && (
            <div className="hidden md:flex items-center gap-3">
              <select
                className="h-8 rounded-md border border-input bg-card px-2 text-sm focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring flex-1 min-w-[130px] max-w-[200px]"
                value={modePaiement}
      ...
      <div className="fixed bottom-0 left-0 right-0 h-16 px-4 bg-card border-t md:hidden flex items-center justify-end gap-3 z-50 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
        <select
          className="h-9 rounded-md border border-input bg-card px-2 text-xs flex-1 min-w-0"
          value={modePaiement}
          onChange={e => setModePaiement(e.target.value)}
          disabled={isWorking}
        >
